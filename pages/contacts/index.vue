<template>
  <div class="p-6 pb-0 pr-0">
    <div class="mb-6 flex justify-between items-center pr-6">
    ...
    <div
      ...
      ref="scrollableRef"
      class="flex flex-col gap-4 max-h-[calc(100vh-144px)] overflow-y-auto scrollable mr-0.25 pb-6"
    >
      <div v-for="(items, letter) in groupedContacts" :key="letter">
        ...
        <div class="flex flex-col gap-4 pr-6">
          <div v-for="(item, index) in items" :key="item._id" class="flex items-center justify-between cursor-pointer">
            ...
            <div class="flex flex-col gap-0.5 cursor-pointer relative" :ref="(el) => setOptionRef(item._id, el)" @click="handleShowOption(item._id)">
              ...
              <Teleport to="body">
                <div
                  ...
                  class="absolute right-0 bg-white rounded-1 shadow-01 w-54 py-2 z-2 duration-300 border border-solid border-secondary-200"
                  :class="showOption === item._id ? 'visible opacity-100 bottom-5' : 'invisible opacity-0 bottom-0'"
                >
                  <div class="flex flex-col">
                    ...
                    >
                      ...
                      <Icon name="ic:baseline-favorite-border" size="16" class="text-secondary-400" />
                    </div>
                    <div
                      @click="handleBlock(item._id)"
                      class="flex justify-between items-center px-6 py-1.5 duration-300 cursor-pointer hover:bg-background-500"
                    >
                      <div class="text-error text-sm">Block</div>
                      <Icon name="ic:baseline-block" size="16" class="text-error" />
                    </div>
                  </div>
                </div>
              </Teleport>
    ...
    <div v-else class="flex flex-col items-center text-secondary">
...
const { getContacts, getFavorites, toggleFavorites } = useUser();
